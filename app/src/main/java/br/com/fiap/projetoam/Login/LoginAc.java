 ...
 */
public class LoginAc extends AppCompatActivity implements LoaderCallbacks<Cursor> {

    private Authentication salesforceAuth = new Authentication();
//    private Authentication salesforceAuth = new Authentication(getApplicationContext());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button mRegister = (Button) findViewById(R.id.btnCadastrar);
    ...
    public void sendMessageLogin(View view) {
        Intent intent = new Intent(this, EstablishmentActivity.class);
        startActivity(intent);
    }*/
    ...
    private boolean mayRequestContacts() {
        ...
        if (shouldShowRequestPermissionRationale(READ_CONTACTS)) {
            Snackbar.make(txtLogin, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
    ...
    private void attemptLogin() {
        ...
        if (TextUtils.isEmpty(email)) {
            ...
            txtLogin.setError(getString(R.string.error_invalid_email));
            focusView = txtLogin;
            cancel = true;
        }
...
//            mAuthTask.execute((Void) null);
    ...
    public void login(String email, final String password) {
        ...
        try {
            jsonBody.put("Senha__c", password);
            final String requestBody = jsonBody.toString();

            request = new StringRequest(Request.Method.GET, salesforceAuth.getUrl() + "/services/data/v43.0/query?q=SELECT+Senha__c+FROM+Empresa__c+WHERE+Email__c+=+'"+email+"'", new Response.Listener<String>() {
                @Override
...
//                    JSONArray array = new JSONArray(response);
                        ...
                        try {
                            if(object.getJSONArray("records").getJSONObject(0).get("Senha__c").toString().equals(password)){
                                Toast.makeText(LoginAc.this, "Sucesso!", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                                startActivity(intent);
                            }
                            else {
                                Toast.makeText(LoginAc.this, "Senha incorreta", Toast.LENGTH_SHORT).show();
                            }
                        }catch (Exception e){
                            Toast.makeText(LoginAc.this, "Usuário não existe", Toast.LENGTH_SHORT).show();
